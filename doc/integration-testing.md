...
4. **Path-computed walls**: `openapi:validate`+`lint`+`drift` when any `openapi/*.yaml` is touched; `pact:consumer`+`provider` when `pact/` is touched **or** the `api` project (the pact provider `aramo-core`) is affected; the nginx build gate (in-image `nginx -t`) + `frontdoor:conf-check` when `deploy/nginx/` is touched.
...
`ci/scripts`, `openapi/`, `pact/`, `deploy/nginx` are not Nx projects, so `nx affected` cannot map them. Each is covered:
...
| `deploy/nginx/**` | No (runtime nginx conf)         | Path-computed pointer; authoritative gate is CI `docker-build(nginx)` (in-image `nginx -t`) + `frontdoor:conf-check`       | None (nothing nx-consumes it) |
