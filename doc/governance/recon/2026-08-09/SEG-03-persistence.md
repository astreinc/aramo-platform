...
- **Reset ESCAPE via migration:** BEFORE-DELETE reject triggers carry an exact-value `app.tenant_reset` escape on PreStartRequirementAudit, PreStartRequirementInstance, placement.OutboxEvent, placement.PlacementProcessEvent (escape migration `20260806090000_placement_tenant_reset_escape`). placement.ContractAssignment has NO trigger/escape (`:123-127`, "NO delete-reject trigger → NO escape migration needed").
...
- **Permanence:** placement.OutboxEvent rows are trigger-immutable (reject_outbox_mutation, update+delete) EXCEPT the `app.tenant_reset` escape → deleted in tenant reset (§2.2.8). Append-only ≠ survives reset.
