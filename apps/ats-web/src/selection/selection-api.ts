...
import type {
  ...
  TransitionSelectionRequest,
  TransitionSelectionResponse,
} from './types';

// R7 selection surface — the FE consumer of the selection backend
// (PR#217 LIST + reads; the mutate endpoints from M5). NOTE the filter
// divergence confirmed at Gate-5: selections filter on talent_id (NOT
// talent_record_id, which is the pipeline filter).
export async function listSelectionsForTalent(
  talentId: string,
): Promise<SelectionListResponse> {
  return apiClient.get<SelectionListResponse>(
    `/v1/selections?talent_id=${encodeURIComponent(talentId)}`,
  );
}

export async function getSelection(id: string): Promise<SelectionView> {
  return apiClient.get<SelectionView>(`/v1/selections/${id}`);
}

export async function listSelectionEvents(
  id: string,
): Promise<SelectionEventsResponse> {
  return apiClient.get<SelectionEventsResponse>(
    `/v1/selections/${id}/events`,
  );
}

// POST /v1/selections/:id/transitions — body { to_state, event_id } +
...
// SELECTION_STATE_INVALID is surfaced via ApiError (the control only
// offers legalNextStates, so the refusal path is defense-in-depth).
export async function transitionSelection(
  id: string,
  body: TransitionSelectionRequest,
  idempotencyKey: string,
): Promise<SelectionView> {
  const res = await apiClient.post<TransitionSelectionResponse>(
    `/v1/selections/${id}/transitions`,
    body,
...
// POST /v1/selections/:id/response — RULING 3: outreach_event_ref_id
// references a prior outreach_sent event (the response picker). 422
// SELECTION_REFERENCE_NOT_FOUND if the ref doesn't resolve.
export async function recordResponse(
  ...
  return apiClient.post<RecordResponseResponse>(
    `/v1/selections/${id}/response`,
    body,
    { headers: { 'Idempotency-Key': idempotencyKey } },
  );
}

// POST /v1/selections/:id/conversation — single conversation_started_at.
export async function recordConversation(
  ...
  return apiClient.post<RecordConversationResponse>(
    `/v1/selections/${id}/conversation`,
    body,
    { headers: { 'Idempotency-Key': idempotencyKey } },
  );
}

// POST /v1/selections/:id/outreach/draft — the GENERATION half of the
...
export async function draftOutreach(
  ...
  return apiClient.post<OutreachDraftResponse>(
    `/v1/selections/${id}/outreach/draft`,
    body,
    { headers: { 'Idempotency-Key': idempotencyKey } },
  );
}

// POST /v1/selections/:id/outreach/send — the DELIVERY half. The
// Idempotency-Key is KEYED ON draft_event_id (stable across send retries →
// dedupes, never double-delivers). On success the selection advances to
...
export async function sendOutreach(
  ...
  return apiClient.post<OutreachSendResponse>(
    `/v1/selections/${id}/outreach/send`,
    body,
    { headers: { 'Idempotency-Key': idempotencyKey } },
  );
}
