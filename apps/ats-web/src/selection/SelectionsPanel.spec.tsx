...
function renderPanel() {
  return render(
    <MemoryRouter>
      <SelectionsPanel talentId="tal-1" />
    </MemoryRouter>,
  );
}

describe('SelectionsPanel', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('fetches selections filtered on talent_id (NOT talent_record_id)', async () => {
    installFetch({
      '/v1/selections': { items: [makeSelection()] },
    ...
    // Row links to the selection-detail view.
    const link = screen.getByRole('link', { name: 'Senior Engineer' });
    expect(link).toHaveAttribute('href', '/selections/eng-1');
  ...
  it('renders the honest empty-state', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/not on any selection yet/i),
      ).toBeInTheDocument(),
    );
  });
  ...
  it('surfaces the error message when the list fetch 403s', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/permission to view selections/i),
      ).toBeInTheDocument(),
    );
  });
});
