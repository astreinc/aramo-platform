...
import { listSelectionsForTalent } from './selection-api';
import { selectionsErrorMessage } from './error-messages';
import { SELECTION_STATE_LABELS, type SelectionView } from './types';

// The Selections tab on talent-detail (§1) — clones PipelinesPanel (the
// per-tab fetch: useState + useEffect([talentId]) + cancelled flag +
// error-message). NOTE the filter divergence: selections filter on
// talent_id (pipelines on talent_record_id).
//
// RULING 4 — the IDs-only view N+1: the row needs the requisition title,
// which SelectionView does not carry. We resolve titles via getRequisition
...
export function SelectionsPanel({ talentId }: { talentId: string }) {
  const [items, setItems] = useState<readonly SelectionView[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    listSelectionsForTalent(talentId)
      .then(async (res) => {
        if (cancelled) return;
        setItems(res.items);
        setLoading(false);
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(selectionsErrorMessage(err));
        setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  if (items.length === 0) {
    return <p>This talent is not on any selection yet.</p>;
  }
  return (
    <ul className="detail__list">
      {items.map((e) => (
        <li key={e.id}>
          <Link to={`/selections/${e.id}`}>
            {titles[e.requisition_id] ?? e.requisition_id}
          </Link>{' '}
          — {SELECTION_STATE_LABELS[e.state]}
        </li>
      ))}
    </ul>
  );
