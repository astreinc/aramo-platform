...
function renderComposer(state: SelectionState = 'engaged') {
  return render(
    <MemoryRouter>
      <OutreachComposer selectionId="eng-1" state={state} onSent={onSent} />
    </MemoryRouter>,
  );
}
...
describe('OutreachComposer', () => {
  ...
  it('PROOF 3 — send POSTs the edited final_text (≠ draft_text — the editable trail)', async () => {
    installFetch({
      ...
      [SEND]: {
        body: {
          selection: { id: 'eng-1', state: 'awaiting_response' },
