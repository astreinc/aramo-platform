...
import type { SelectionEventView, SelectionView } from './types';
...
function makeEvent(
  id: string,
  event_type: SelectionEventView['event_type'],
...
function renderAt(session: Session) {
  return render(
    <MemoryRouter initialEntries={['/selections/eng-1']}>
      <Routes>
        <Route
          path="/selections/:selectionId"
          element={<SelectionDetailView sessionOverride={session} />}
        />
...
describe('SelectionDetailView', () => {
  ...
  it('a read-only actor sees the event log but no controls (scope-gating)', async () => {
    ...
    expect(
      screen.queryByRole('heading', { name: 'Move selection' }),
    ).toBeNull();
    expect(
  ...
  it('a write actor sees all controls and the event log renders the 5 types', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Move selection' })).toBeInTheDocument();
  ...
  it('surfaces the detail error when the selection fetch 404s', async () => {
    installFetch({
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/this selection is not available/i),
      ).toBeInTheDocument(),
    );
  });
});
