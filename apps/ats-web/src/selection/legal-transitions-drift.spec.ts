...
import { SELECTION_STATE_VALUES, type SelectionState } from './types';
...
// because the selection matrix is an inline const inside canTransition(),
...
function parseBeMatrix(source: string): Record<SelectionState, string[]> {
  ...
  return result as Record<SelectionState, string[]>;
}

describe('selection legal-transitions drift smoke spec', () => {
  it('the FE mirror is structurally deep-equal to the BE ALLOWED matrix', () => {
    ...
    expect(beKeys.length).toBe(SELECTION_STATE_VALUES.length);
  ...
  it('every target referenced is a valid SelectionState', () => {
    for (const targets of Object.values(LEGAL_TRANSITIONS)) {
      for (const t of targets) {
        expect(SELECTION_STATE_VALUES).toContain(t);
      }
    }
  });
});
