...
import { SELECTION_STATE_LABELS } from './types';

describe('SelectionTransitionControl', () => {
  it('renders only the legal targets for the current state (load-bearing)', () => {
    render(
      <SelectionTransitionControl from="evaluated" onSubmit={vi.fn()} />,
    );
    ...
    for (const target of LEGAL_TRANSITIONS.evaluated) {
      expect(
        within(menu).getByRole('menuitem', {
          name: SELECTION_STATE_LABELS[target],
        }),
      ).toBeInTheDocument();
    }
    ...
    expect(
      within(menu).queryByRole('menuitem', {
        name: SELECTION_STATE_LABELS.evaluated,
      }),
    ).not.toBeInTheDocument();
    // A non-adjacent state is NOT offered.
    expect(
      within(menu).queryByRole('menuitem', {
        name: SELECTION_STATE_LABELS.submitted,
      }),
    ).not.toBeInTheDocument();
  });

  it('renders the Final badge instead of the trigger for terminal states', () => {
    render(<SelectionTransitionControl from="submitted" onSubmit={vi.fn()} />);
    expect(
  ...
  it('submits the selected target via onSubmit when confirmed', () => {
    ...
    render(<SelectionTransitionControl from="engaged" onSubmit={onSubmit} />);

    fireEvent.click(screen.getByRole('button', { name: 'Move to…' }));
    fireEvent.click(
      screen.getByRole('menuitem', {
        name: SELECTION_STATE_LABELS.awaiting_response,
      }),
    );
