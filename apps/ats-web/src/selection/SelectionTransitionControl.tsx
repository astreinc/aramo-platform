...
import { SELECTION_STATE_LABELS, type SelectionState } from './types';

interface SelectionTransitionControlProps {
  readonly from: SelectionState;
  readonly disabled?: boolean;
  readonly onSubmit: (to: SelectionState) => Promise<void>;
}
...
// selection transition body is { to_state, event_id } only.
//
// A11y: trigger is role=button; the panel sets role=menu (Radix Popover
// handles outside-click + escape + focus return).
export function SelectionTransitionControl({
  from,
  disabled,
  onSubmit,
}: SelectionTransitionControlProps) {
  ...
  if (targets.length === 0) {
    return (
      <span
        className="selection-transition__terminal"
    ...
    <RadixPopover.Root open={open} onOpenChange={setOpen}>
      ...
      <RadixPopover.Portal>
        <RadixPopover.Content
          ...
          align="start"
          sideOffset={6}
          aria-label="Move selection to a new state"
        >
          {selected === null ? (
            ...
            >
              <p>
                Move to <strong>{SELECTION_STATE_LABELS[selected]}</strong>?
              </p>
              {error !== null ? (
