...
// per row to fetch.
//
// Legacy-Pipeline-Canonicalization — `current_stage` is the raw Pipeline stage
// (canonical 7-state) directly. The retired L2-E "submitted overlay" that
// reproduced a `submitted` current_stage from Submittal history is removed;
// the submitted-to-client fact is Submittal-owned and is not projected onto the
// Pipeline current_stage read-model.
@Injectable()
export class TalentRecordEnrichmentService {
  ...
  async crossFacets(
    ...
    // stage — the raw current active stage label (canonical 7-state); 'none'
    // when not in any active flow.
