...
const MIGRATIONS = [
  ...
  M('libs/documents/prisma/migrations/20260921180000_init_documents_model/migration.sql'),
  M('libs/documents/prisma/migrations/20260922130000_doc2_templates_rendering_requirements/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260519170000_init_talent_evidence_model/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260922120000_doc1b_talentdocument_reconciliation/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260714120000_tr7_b1_education_certification/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260915120000_hf1_resume_provenance/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260915170000_hf2_experience_intelligence/migration.sql'),
  M('libs/talent-evidence/prisma/migrations/20260915180000_skill_tax_1g_canonical_reconciliation/migration.sql'),
  ...
  M('libs/submittal/prisma/migrations/20260920130000_talent_intel_1d_d_submittal_resume_edition/migration.sql'),
...
describe.skipIf(process.env['ARAMO_RUN_INTEGRATION'] !== '1')(
  'POST /v1/selections/{id}/outreach/draft + /send — HTTP integration (real Postgres 17)',
  () => {
    ...
    function draftOutreach(
      ...
      return fetch(`http://127.0.0.1:${port}/v1/selections/${id}/outreach/draft`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${recruiterJwt}`,
    ...
    it('SEND: SELECTION_REFERENCE_NOT_FOUND 422 when draft_event_id is unknown', { timeout: 60_000 }, async () => {
      const id = await createAndAdvanceToEngaged();
    ...
    it('INSUFFICIENT_PERMISSIONS 403 with portal JWT (both draft + send)', { timeout: 30_000 }, async () => {
      const draftRes = await fetch(
        `http://127.0.0.1:${port}/v1/selections/00000000-0000-7000-8000-000000000aaa/outreach/draft`,
        {
          method: 'POST',
          headers: {
      ...
      const sendRes = await fetch(
        `http://127.0.0.1:${port}/v1/selections/00000000-0000-7000-8000-000000000aaa/outreach/send`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${portalJwt}`,
