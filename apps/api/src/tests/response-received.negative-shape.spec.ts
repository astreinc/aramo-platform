...
const AUDIENCE = 'aramo-response-received-neg-shape';
const ALG = 'RS256';
...
describe.skipIf(process.env['ARAMO_RUN_INTEGRATION'] !== '1')(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      // Step 5.5 resolves 'allowed' (resolver requires profile_storage +
      // matching granted before contacting can be checked).
    ...
    it('200 response-received response contains no Match-Class vocabulary keys anywhere', { timeout: 60_000 }, async () => {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/v1/selections/${selectionId}/response`, {
        ...
        body: JSON.stringify({
          response_received_at: '2026-05-25T12:00:00.000Z',
      ...
      expect(
        hits,
        `Match-Class vocabulary leaked into response-received response: ${hits.map((h) => h.path).join(', ')}`,
      ).toEqual([]);
    });
  },
);
