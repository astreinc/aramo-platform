...
// M4 PR-4 §4.8 — Companion negative-shape integration test for POST
// /v1/submittals/{submittal_id}/confirm. F23 standing pattern: boot the
// AppModule, seed an Entrustable examination + draft submittal pinned to
// it, POST the confirm request with all attestations true, and walk the
// 200 response body recursively asserting that no forbidden Match-Class
...
const EXAM_ID = '11110000-0000-7000-8000-0000000e0005';
...
describe.skipIf(process.env['ARAMO_RUN_INTEGRATION'] !== '1')(
  'POST /v1/submittals/{id}/confirm — negative-shape (no Match-Class vocabulary leak)',
  () => {
    ...
    beforeAll(async () => {
      ...
      // Seed the active requisition + the Entrustable examination pinned by
      ...
      recruiterJwt = await new SignJWT({
        ...
        actor_kind: 'user',
        tenant_id: TENANT_ID,
        authz_version: __authzTestResolver.grant(TENANT_ID, RECRUITER_ID, ['submittal:create', 'submittal:approve']),
      })
    ...
    it('200 confirm response contains no Match-Class vocabulary keys anywhere', async () => {
      // First create a draft submittal so we have an id to confirm.
      ...
      expect(
        hits,
        `Match-Class vocabulary leaked into POST /v1/submittals/:id/confirm response: ${hits
          .map((h) => `${h.path}`)
          .join(', ')}`,
      ).toEqual([]);
    });
