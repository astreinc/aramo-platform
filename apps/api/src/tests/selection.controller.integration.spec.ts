...
// M5 PR-4 §4.11 — SelectionController integration spec.
//
...
// JwtAuthGuard → @AuthContext + class-validator → SelectionController
// → SelectionRepository → Postgres → response).
//
// Scope per directive §4.11:
//   POST /v1/selections:
//     - happy: 201 + selection row + initial event row persisted.
...
const TALENT_EVIDENCE_HF2 = resolve(ROOT, 'libs/talent-evidence/prisma/migrations/20260915170000_hf2_experience_intelligence/migration.sql');
const TALENT_EVIDENCE_1G = resolve(ROOT, 'libs/talent-evidence/prisma/migrations/20260915180000_skill_tax_1g_canonical_reconciliation/migration.sql');
...
const SELECTION_INIT = resolve(ROOT, 'libs/selection/prisma/migrations/20260525120000_init_selection_model/migration.sql');
...
const AUDIENCE = 'aramo-selection-controller-spec';
const ALG = 'RS256';
...
describe.skipIf(process.env['ARAMO_RUN_INTEGRATION'] !== '1')(
  'SelectionController — HTTP integration (real Postgres 17)',
  () => {
    ...
    let oldSelectionJwt: string; // lacks selection:read — guard rejects (post-rename boundary)
    ...
    it('POST /v1/selections happy: 201 + selection + event rows persisted', async () => {
      ...
      const body = (await res.json()) as { selection: { id: string; state: string } };
      expect(body.selection.state).toBe('surfaced');
      // Verify event row exists.
      const evRows = await setupClient.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM selection."TalentSelectionEvent" WHERE selection_id = $1::uuid`,
        [body.selection.id],
      );
    ...
    it('POST /v1/selections/{id}/transitions happy: surfaced → evaluated', async () => {
      ...
      const res = await fetch(
        `http://127.0.0.1:${port}/v1/selections/${selectionId}/transitions`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${recruiterJwt}`,
    ...
    it('GET /v1/selections/{id}: 200 happy + 404 unknown', async () => {
      ...
      const hit = await fetch(
        `http://127.0.0.1:${port}/v1/selections/${createBody.selection.id}`,
    ...
    it('GET /v1/selections/{id}/events: 200 with at least the initial event + 404 unknown selection', async () => {
      const createRes = await fetch(`http://127.0.0.1:${port}/v1/selections`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${recruiterJwt}`,
      ...
      const hit = await fetch(
        `http://127.0.0.1:${port}/v1/selections/${createBody.selection.id}/events`,
        { headers: { Authorization: `Bearer ${recruiterJwt}` } },
      );
      expect(hit.status).toBe(200);
