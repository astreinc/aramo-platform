...
//   - token_use === "id".
//
// Error CLASSING (§5 Auth-Hardening D2 P4): token-content rejections
...
//
// email_verified normalization (§5 Auth-Hardening D2 P1, verifier-side):
// Cognito emits `email_verified` as a real boolean for native users, but a
// federated IdP (Google/Microsoft) surfaces it as the STRING "true". We
// accept the string ONLY when the token presents a trusted-federation
// provider signal (the `identities[].providerName` / `cognito:username`
// prefix, matched against AUTH_TRUSTED_IDP_NAMES). Native / untrusted tokens
// keep the strict boolean `=== true` gate. The gate is PARSED (we read the
// IdP's verified assertion), never REMOVED — an unverified native email, or
// a string assertion from an untrusted/unconfigured IdP, still FAILS. An
// empty AUTH_TRUSTED_IDP_NAMES trusts nothing (fail-closed). This is the
// unverified-email account-takeover vector, and it stays closed.
...
export interface CognitoIdTokenClaims extends JWTPayload {
  ...
  token_use?: string;
  // Federated-provider signals (present only on federated tokens). They
  // scope the string-normalization to trusted federation.
...
export class CognitoVerifierService {
  ...
  //     untrusted/native token) FAILS — the gate is intact, fail-closed.
  ...
  // email_verified never passes (fail-closed).
