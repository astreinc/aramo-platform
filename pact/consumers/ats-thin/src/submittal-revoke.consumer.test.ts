...
describe('ATS thin consumer → POST /v1/submittals/{id}/revoke', () => {
  ...
  const REPLAY_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b7110';
  const CONFLICT_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b7111';

  it('idempotency replay: same key + same body returns cached 200 response', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a prior submittal-revoke response')
      .uponReceiving('a submittal-revoke request replaying a prior key + body')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_SUBMITTED}/revoke`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
      ...
      .willRespondWith(200, (b) => {
        b.headers({ 'X-Request-ID': uuid(REQUEST_ID) }).jsonBody({
          submittal: like({ id: SUBMITTAL_ID_SUBMITTED, state: 'revoked' }),
          evidence_package_mutated: false,
        });
      })
      .executeTest(async (mock) => {
        const res = await fetch(`${mock.url}/v1/submittals/${SUBMITTAL_ID_SUBMITTED}/revoke`, {
          method: 'POST',
          headers: {
            Authorization: 'Bearer eyJfake.token',
            'X-Request-ID': REQUEST_ID,
  ...
  it('idempotency conflict: same key + different body returns 409 IDEMPOTENCY_KEY_CONFLICT', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a different submittal-revoke body')
      .uponReceiving('a submittal-revoke request with a conflicting prior key')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_SUBMITTED}/revoke`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
