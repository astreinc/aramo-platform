...
describe('ATS thin consumer → POST /v1/submittals/{id}/confirm-ats', () => {
  ...
  const REPLAY_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b8310';
  const CONFLICT_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b8311';

  it('idempotency replay: same key + same body returns cached 200 response', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a prior submittal-confirm-ats response')
      .uponReceiving('a submittal-confirm-ats request replaying a prior key + body')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_HAPPY}/confirm-ats`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
      ...
      .willRespondWith(200, (b) => {
        b.headers({ 'X-Request-ID': uuid(REQUEST_ID) }).jsonBody({
          submittal: like({ id: SUBMITTAL_ID_HAPPY, state: 'confirmed' }),
        });
      })
      .executeTest(async (mock) => {
        const res = await fetch(`${mock.url}/v1/submittals/${SUBMITTAL_ID_HAPPY}/confirm-ats`, {
          method: 'POST',
          headers: {
            Authorization: 'Bearer eyJfake.token',
            'X-Request-ID': REQUEST_ID,
  ...
  it('idempotency conflict: same key + different body returns 409 IDEMPOTENCY_KEY_CONFLICT', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a different submittal-confirm-ats body')
      .uponReceiving('a submittal-confirm-ats request with a conflicting prior key')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_HAPPY}/confirm-ats`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
