...
describe('ATS thin consumer → POST /v1/submittals/{id}/mark-ready', () => {
  ...
  // Uses EMPTY_BODY ({ _placeholder: true } per PL-67 workaround).
  const REPLAY_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b8110';
  const CONFLICT_KEY = '0190d5a4-7e01-7e2a-a4d3-3d4f1c2b8111';

  it('idempotency replay: same key + same body returns cached 200 response', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a prior submittal-mark-ready response')
      .uponReceiving('a submittal-mark-ready request replaying a prior key + body')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_HAPPY}/mark-ready`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
      ...
      .willRespondWith(200, (b) => {
        b.headers({ 'X-Request-ID': uuid(REQUEST_ID) }).jsonBody({
          submittal: like({ id: SUBMITTAL_ID_HAPPY, state: 'ready_for_review' }),
        });
      })
      .executeTest(async (mock) => {
        const res = await fetch(`${mock.url}/v1/submittals/${SUBMITTAL_ID_HAPPY}/mark-ready`, {
          method: 'POST',
          headers: {
            Authorization: 'Bearer eyJfake.token',
            'X-Request-ID': REQUEST_ID,
  ...
  it('idempotency conflict: same key + different body returns 409 IDEMPOTENCY_KEY_CONFLICT', async () => {
    await provider
      .addInteraction()
      .given('an idempotency key has been recorded with a different submittal-mark-ready body')
      .uponReceiving('a submittal-mark-ready request with a conflicting prior key')
      .withRequest('POST', `/v1/submittals/${SUBMITTAL_ID_HAPPY}/mark-ready`, (b) => {
        b.headers({
          Authorization: like('Bearer eyJfake.token'),
          'X-Request-ID': uuid(REQUEST_ID),
