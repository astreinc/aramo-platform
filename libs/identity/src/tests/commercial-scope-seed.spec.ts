...
describe('Company-Fields v1.1 â company:read_commercial catalog parity', () => {
  ...
  it('SEED_SCOPE_KEYS is 132 (107 + 4 Track4/T4-D assignment + 1 Slice#3 assignment:extend + 2 Track5/T5-P1 assignment:commercials + 2 Track8/T8-P2 requisition:import + 2 Track7/T7-P1 placement:permanent + 1 Track7/T7-P2 placement:remedy:resolve + 1 Track7/T7-P3 placement:permanent:terms:write + 2 Track8/T8-CONNECTOR-A integration + 1 L1-A requisition:create:establish + 4 COMM-B2 communication)', () => {
    expect(SEED_SCOPE_KEYS).toHaveLength(158); // CSP PR-2 +3 client-submittal-policy:{read,write,override} → 155+3=158. DOC-3 +1 document:execute → 154+1=155. DOC-2 +4 (document_template:{read,manage}, document_requirement:{read,manage}) → 150+4=154. DOC-1a +3 document scopes → 147+3=150. // TI-1D-D +1 pipeline:resume:set (dedicated Requisition-context resume-select mutation scope) -> 146+1=147. SKILL-TAX-1F-A +2 platform:skill:{read,manage} → 144+2=146. talent:edit:contact +1 (owner/admin contact-anchor edit) → 143+1=144. WL-B2 +1 address:lookup (dedicated address-lookup scope) -> 143. COMM PART A +1 engagement:policy:override -> 142. COMM-C2B +2 communication:email:send + communication:meeting:create → 141. COMM-C3 +2 engagement:policy:read + engagement:policy:write → 139. L5-P6 +1 pre_start_requirement:verify → 137. L6-0 −2 assignment:create + assignment:update (grounded-dead removed) -> 138−2=136. L4/P5 +2 offer:read + offer:read:financial -> 136+2=138. L2-I (D1) +1 integration:pipeline-mapping:write -> 135+1=136. L2-F +3 client-selection:create/read/transition; HYG-1 -3 dead orphan scopes.
  });
});
