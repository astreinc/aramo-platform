...
const richDb = (): FakeOpts => ({
  ...
  archiveRows: {
    ...
    'engagement."TalentSubmittalEvent"': [{ id: 'se1' }],
    'engagement."TalentSubmittalRecord"': [{ id: 'sr1' }],
...
describe('TenantResetService — happy path + delete ordering (§2.2 / §3.8)', () => {
  it('deletes in §2.2 order, child-before-parent, then COMMITs COMPLETED', async () => {
    ...
    const expectedOrder = [
      ...
      'DELETE FROM engagement."TalentSubmittalEvent"',
      'DELETE FROM engagement."TalentSubmittalRecord"',
