...
//
// REQUISITION_NO_OPENINGS (HTTP 409) was the pipeline-transition
// over-capacity refusal — RETIRED at T4-B2 §7: pipeline no longer owns
// requisition capacity (capacity is DERIVED from the active
// ContractAssignment population), so a `placed` transition never
// decrements openings and never refuses on over-capacity. The code is
// RESERVED / no longer emitted — kept in the registry for compatibility
// (still mapped by the ats-web pipeline error-message map). See
...
export const ERROR_CODES = [
  ...
  'ASSIGNMENT_EXTENSION_NOT_FORWARD',  // Slice #3 (Assignment-Extension, R-EXTEND-TXN) — an extend refused because new_expected_end_at is not strictly forward: not in the future, or not after the current expected end (or the actual start when none is set). HTTP 422. A business-rule refusal (never a DB-constraint translation) — details.reason: `not_future` or `not_forward`. An extension only ever moves the planned end forward; a backward/equal/past target is a correction (a future distinct operation), not an extension.
  ...
  // (Legacy-Pipeline-Canonicalization); no throw-site remains. Kept because the error-code
  // registry is APPEND-ONLY (see RESERVED_CODES in ci/scripts/verify-dead-error-codes.ts).
