...
// Per-module PrismaService for the documents module (DOC-1a).
//
...
//   3. $connect override with `validated` memoization; byte-identical error
//      'DATABASE_URL is not configured' on absent env.
//   4. OnModuleDestroy -> $disconnect.
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger = new Logger('PrismaService (documents)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (documents) connected');
  }
