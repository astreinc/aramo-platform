...
export class ConsentController {
  ...
  // PR-5: GET /consent/state/{talent_record_id}. Informational read endpoint;
  // no Idempotency-Key handling (Phase 1 §6: N/A for read endpoints).
  // No request body. talent_record_id is a path parameter; UUID format
  // validated explicitly because @Param doesn't run class-validator.
  @Get('state/:talent_record_id')
  @HttpCode(HttpStatus.OK)
  async getTalentConsentState(
    ...
    this.assertTalentIdFormat(talent_record_id, requestId);
    return this.consentService.getState(talent_record_id, authContext, requestId);
  }

  // PR-6: GET /consent/history/{talent_record_id}. Informational read endpoint;
  ...
  // PR-7: GET /consent/decision-log/{talent_record_id}. Informational read
  ...
  private assertTalentIdFormat(talent_record_id: string, requestId: string): void {
    if (!UUID_REGEX.test(talent_record_id)) {
      throw new AramoError(
        'VALIDATION_ERROR',
        'talent_record_id must be a UUID',
        400,
        { requestId, details: { invalid_field: 'talent_record_id' } },
      );
    }
  }
