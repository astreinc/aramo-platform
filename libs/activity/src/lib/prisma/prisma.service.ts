...
// Per-module PrismaService for the activity module (PR-A5a Gate 5).
// Same lazy-validation pattern as the workspace norm (A2 §3 of the
// pattern of record).
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger = new Logger('PrismaService (activity)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (activity) connected');
  }
