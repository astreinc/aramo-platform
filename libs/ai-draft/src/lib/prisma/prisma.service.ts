...
import { PrismaClient } from '../../../node_modules/.prisma/ai-draft-client/client.js';

// Per-module PrismaService for the ai-draft module (M5 PR-5). Twelfth
// model-bearing PrismaService in the workspace, joining the eleven from
...
// existing test instantiation pattern across the sibling libs while
// routing emissions through the structured AramoLogger pipeline.
// Lifecycle hooks (onModuleDestroy) unaffected.

@Injectable()
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger: AramoLogger = createAramoLogger('PrismaService (ai-draft)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log({ event: 'prisma_service_connected', surface: 'ai-draft' });
  }
