...
// Per-module PrismaService for the attachment module (PR-A4 Gate 5).
// Lazy-validation pattern per the workspace norm.
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger = new Logger('PrismaService (attachment)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (attachment) connected');
  }
