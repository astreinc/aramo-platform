...
// Per-module PrismaService for the communications module (COMM-B1).
//
// Lazy-validation pattern per the workspace norm (mirrors the integration
// module). Four-step contract:
...
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger = new Logger('PrismaService (communications)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (communications) connected');
  }
