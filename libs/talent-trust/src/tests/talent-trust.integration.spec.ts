...
// TR-8 D2 — TrustState.verified_control_stale (the regenerated client SELECTs it).
const VERIFIED_STALE_MIGRATION_PATH = resolve(
  __dirname,
  ...
  () => {
    ...
    it('contradiction caps the dimension at CORROBORATED, raises the count, and writes a CONTRADICTS link', async () => {
      const authoritative = await service.recordEvidence({
        ...
        assertion_payload: { work_authorization_status_raw: 'US_CITIZEN', requires_sponsorship: false },
        source_class: 'AUTHORITATIVE_ISSUER',
        method: 'DOCUMENT',
      ...
      const contradicting = await service.recordEvidence({
        ...
        dimension: 'ELIGIBILITY',
        assertion_type: 'RIGHT_TO_WORK',
        assertion_payload: { work_authorization_status_raw: 'REQUIRES_SPONSORSHIP', requires_sponsorship: true },
