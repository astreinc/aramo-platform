...
// Per-module PrismaService for the talent-trust module (TR-1). Wraps the
// talent-trust Prisma client; each module owns its own generated client
...
// the `$connect` override, preserving the byte-identical
// `'DATABASE_URL is not configured'` message. Mirrors libs/talent-evidence.
//
// @Optional() on databaseUrl so Nest DI does not try to resolve a String
// token (guarded by apps/api/src/tests/app-module-di.spec.ts). Tests may pass
// an explicit URL; production wiring relies on process.env['DATABASE_URL'].
@Injectable()
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (talent-trust) connected');
  }
