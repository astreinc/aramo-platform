...
// Per-module PrismaService for the portal-identity module (Portal P1). Wraps
// the portal-identity Prisma client; each module owns its own generated
// client (ADR-0001 D3 / Architecture v2.x §7 schema-per-module). Mirrors
// libs/identity-index's PrismaService exactly.
//
...
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (portal-identity) connected');
  }
