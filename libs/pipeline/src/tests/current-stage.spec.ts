...
describe('PipelineRepository.findCurrentStageForTalentIds', () => {
  it('picks the most-advanced ACTIVE stage per talent; tie-break = lowest req_id', async () => {
    const findMany = vi.fn().mockResolvedValue([
      { talent_record_id: 't1', requisition_id: 'rB', status: 'qualifying' },
      { talent_record_id: 't1', requisition_id: 'rA', status: 'qualified' }, // more advanced
    ...
    expect(m.get('t1')).toEqual({ stage: 'qualified', requisition_id: 'rA' });
  ...
  it('constrains the query to ACTIVE stages and the visible-requisition set', async () => {
    ...
    expect(where.status.in).toContain('qualified');
    expect(where.status.in).not.toContain('completed'); // terminal excluded
    expect(where.status.in).not.toContain('not_in_consideration'); // terminal excluded
