...
// Per-module PrismaService for the conversation_transcript module (CI-B3).
//
// Lazy-validation pattern per the workspace norm (mirrors communications /
// integration). Four-step contract:
...
export class PrismaService extends PrismaClient implements OnModuleDestroy {
  private readonly logger = new Logger('PrismaService (conversation-transcript)');
  ...
  override async $connect(): Promise<void> {
    ...
    await super.$connect();
    this.logger.log('PrismaService (conversation-transcript) connected');
  }
