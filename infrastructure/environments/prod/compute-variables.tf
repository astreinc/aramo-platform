# Aramo Step-4 Directive 2 — compute-layer variables (prod).
...
variable "api_image_tag" {
  description = "ECR image tag for the api service (e.g. a release git SHA). Defaults to 'latest' for the rolling-tag first deploy; pin to a digest/SHA for prod releases."
  type        = string
  default     = "latest"
}
...
variable "api_cpu" {
  description = "Fargate CPU units for the api task (override higher for prod via tfvars)."
  type        = number
  default     = 512
}
...
variable "redis_node_type" {
  description = "ElastiCache node type for the BullMQ Redis (override higher for prod via tfvars)."
  type        = string
