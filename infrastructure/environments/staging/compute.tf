# Aramo Step-4 Directive 2 — compute / run layer (staging composition).
#
...
# + ECS cluster + ALB + the two Fargate services. Staging mirrors prod (the
# §5-checklist rehearsal ground); see prod/compute.tf for the same wiring.
#
# ★ Account-independent authoring. validate-clean now; apply-and-prove is
# gated on the AWS account (the data plane has never been applied either —
# first-apply is the real proof). Apply incrementally: data plane first,
# then this compute layer on top.
...
locals {
  ...
  # Terraform-derived config merged with operator-supplied per-env values
  # (Cognito URLs, redirect/sign-out, audience, etc. land in *_extra_env via
  # tfvars at account setup — see terraform.tfvars.example).
  ...
  # Execution-role-injected secrets (task-def `secrets`; values out-of-band).
  api_secrets = {
