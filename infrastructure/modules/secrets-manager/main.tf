...
# existing `aramo/<env>/<name>` convention.
#
...
# — never baked into an image or into state.
#
# NOTE: TENANT-LLM-1 retired the platform-wide `aramo/<env>/anthropic-api-key`.
# Per-tenant BYO Anthropic keys live at
# `aramo/<env>/tenant-llm/<tenant_id>/anthropic-api-key`, created + read by the
# app at runtime — they are NOT TF-managed containers and do not belong in
# `secret_names`.
